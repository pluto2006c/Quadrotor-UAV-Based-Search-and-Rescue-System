...
namespace
{
constexpr double kStartupSleepSec = 1.0;
constexpr double kServiceReadySleepSec = 0.5;
constexpr double kRcWaitSleepSec = 0.1;
constexpr double kConnectionRetrySleepSec = 1.0;
constexpr int kDefaultQueueSize = 10;
constexpr int kFastQueueSize = 100;
constexpr int kConnectionWarnTrials = 5;
} // namespace
...
int main(int argc, char *argv[])
{
    ...
    ros::Duration(kStartupSleepSec).sleep();
    ...
    fsm.ctrl_FCU_pub = nh.advertise<mavros_msgs::AttitudeTarget>("/mavros/setpoint_raw/attitude", kDefaultQueueSize);
    fsm.traj_start_trigger_pub = nh.advertise<geometry_msgs::PoseStamped>("/traj_start_trigger", kDefaultQueueSize);

    fsm.debug_pub = nh.advertise<quadrotor_msgs::Px4ctrlDebug>("/debugPx4ctrl", kDefaultQueueSize); // debug
    ...
    ros::Duration(kServiceReadySleepSec).sleep();
    ...
    else
    {
        ...
        while (ros::ok())
        {
            ...
            ros::Duration(kRcWaitSleepSec).sleep();
        }
    }
    ...
    {
        ros::spinOnce();
        ros::Duration(kConnectionRetrySleepSec).sleep();
        if (trials++ > kConnectionWarnTrials)
