...
namespace
{
constexpr double kRcStickCenter = 1500.0;
constexpr double kRcStickScale = 500.0;
constexpr double kRcSwitchMin = 1000.0;
constexpr double kRcSwitchScale = 1000.0;

double normalizeStickChannel(double raw)
{
    double value = (raw - kRcStickCenter) / kRcStickScale;
    if (value > RC_Data_t::DEAD_ZONE)
        return (value - RC_Data_t::DEAD_ZONE) / (1 - RC_Data_t::DEAD_ZONE);
    if (value < -RC_Data_t::DEAD_ZONE)
        return (value + RC_Data_t::DEAD_ZONE) / (1 - RC_Data_t::DEAD_ZONE);
    return 0.0;
}

double normalizeSwitchChannel(double raw)
{
    return (raw - kRcSwitchMin) / kRcSwitchScale;
}
} // namespace
...
void RC_Data_t::feed(mavros_msgs::RCInConstPtr pMsg)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ch[i] = normalizeStickChannel((double)msg.channels[i]);
    }

    mode = normalizeSwitchChannel((double)msg.channels[4]);
    gear = normalizeSwitchChannel((double)msg.channels[5]);
    reboot_cmd = normalizeSwitchChannel((double)msg.channels[7]);
