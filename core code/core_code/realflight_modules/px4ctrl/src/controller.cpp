...
namespace
{
constexpr size_t kMaxTimedThrustSamples = 100;
constexpr double kMinThrustDelaySec = 0.035;
constexpr double kMaxThrustDelaySec = 0.045;

Eigen::Vector3d makeGainVector(double x, double y, double z)
{
  Eigen::Vector3d gain;
  gain << x, y, z;
  return gain;
    ...
    Controller_Output_t &u)
{
  /* WRITE YOUR CODE HERE */
      ...
      Eigen::Vector3d Kp = makeGainVector(param_.gain.Kp0, param_.gain.Kp1, param_.gain.Kp2);
      Eigen::Vector3d Kv = makeGainVector(param_.gain.Kv0, param_.gain.Kv1, param_.gain.Kv2);
    ...
    const Parameter_t &param)
{
  ...
  while (timed_thrust_.size() >= 1)
  {
    ...
    if (time_passed > kMaxThrustDelaySec) // 45ms
    {
      // printf("continue, time_passed=%f\n", time_passed);
      timed_thrust_.pop();
      continue;
    }
    if (time_passed < kMinThrustDelaySec) // 35ms
    {
