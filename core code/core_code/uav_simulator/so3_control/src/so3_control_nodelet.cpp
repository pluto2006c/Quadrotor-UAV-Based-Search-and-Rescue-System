...
namespace
{
constexpr double kMinCommandGain = 1e-5;
constexpr double kInitStateDisabledZ = -9999.0;
constexpr int kDefaultQueueSize = 10;
constexpr int kMotorQueueSize = 2;

bool hasPositiveGain(const boost::array<double, 3>& gain)
{
  return gain[0] > kMinCommandGain || gain[1] > kMinCommandGain || gain[2] > kMinCommandGain;
}
} // namespace
  ...
  const quadrotor_msgs::PositionCommand::ConstPtr& cmd)
{
  ...
  if (hasPositiveGain(cmd->kx))
  {
    kx_ = Eigen::Vector3d(cmd->kx[0], cmd->kx[1], cmd->kx[2]);
  }
  if (hasPositiveGain(cmd->kv))
  {
...
SO3ControlNodelet::odom_callback(const nav_msgs::Odometry::ConstPtr& odom)
{
  ...
  else if ( init_z_ > kInitStateDisabledZ )
  {
...
SO3ControlNodelet::onInit(void)
{
  ...
  so3_command_pub_ = n.advertise<quadrotor_msgs::SO3Command>("so3_cmd", kDefaultQueueSize);

  odom_sub_ = n.subscribe("odom", kDefaultQueueSize, &SO3ControlNodelet::odom_callback, this,
                          ros::TransportHints().tcpNoDelay());
  position_cmd_sub_ =
    n.subscribe("position_cmd", kDefaultQueueSize, &SO3ControlNodelet::position_cmd_callback,
                this, ros::TransportHints().tcpNoDelay());

  enable_motors_sub_ =
    n.subscribe("motors", kMotorQueueSize, &SO3ControlNodelet::enable_motors_callback, this,
                ros::TransportHints().tcpNoDelay());
  corrections_sub_ =
    n.subscribe("corrections", kDefaultQueueSize, &SO3ControlNodelet::corrections_callback,
                this, ros::TransportHints().tcpNoDelay());

  imu_sub_ = n.subscribe("imu", kDefaultQueueSize, &SO3ControlNodelet::imu_callback, this,
                         ros::TransportHints().tcpNoDelay());
}
