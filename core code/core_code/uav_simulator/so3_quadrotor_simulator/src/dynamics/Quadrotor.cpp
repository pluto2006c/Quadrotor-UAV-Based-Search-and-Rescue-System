...
namespace
{
constexpr int kInternalStateSize = 22;
constexpr int kPositionStateOffset = 0;
constexpr int kVelocityStateOffset = 3;
constexpr int kRotationCol0Offset = 6;
constexpr int kRotationCol1Offset = 9;
constexpr int kRotationCol2Offset = 12;
constexpr int kOmegaStateOffset = 15;
constexpr int kMotorStateOffset = 18;
...
Quadrotor::step(double dt)
{
  ...
  for (int i = 0; i < kInternalStateSize; ++i)
  {
    if (std::isnan(internal_state_[i]))
    {
      std::cout << "dump " << i << " << pos ";
      for (int j = 0; j < kInternalStateSize; ++j)
      {
        std::cout << save[j] << " ";
      }
      std::cout << std::endl;
  ...
  for (int i = 0; i < 3; i++)
  {
    state_.x(i) = internal_state_[kPositionStateOffset + i];
    state_.v(i) = internal_state_[kVelocityStateOffset + i];
    state_.R(i, 0) = internal_state_[kRotationCol0Offset + i];
    state_.R(i, 1) = internal_state_[kRotationCol1Offset + i];
    state_.R(i, 2) = internal_state_[kRotationCol2Offset + i];
    state_.omega(i) = internal_state_[kOmegaStateOffset + i];
  }
  state_.motor_rpm(0) = internal_state_[kMotorStateOffset + 0];
  state_.motor_rpm(1) = internal_state_[kMotorStateOffset + 1];
  state_.motor_rpm(2) = internal_state_[kMotorStateOffset + 2];
  state_.motor_rpm(3) = internal_state_[kMotorStateOffset + 3];
  ...
  if (state_.x(2) < kFloorHeight && state_.v(2) < 0)
  {
    state_.x(2) = kFloorHeight;
    state_.v(2) = 0;
  }
  updateInternalState();
}
                      ...
                      Quadrotor::InternalState& dxdt, const double /* t */)
{
  ...
  for (int i = 0; i < 3; i++)
  {
    cur_state.x(i) = x[kPositionStateOffset + i];
    cur_state.v(i) = x[kVelocityStateOffset + i];
    cur_state.R(i, 0) = x[kRotationCol0Offset + i];
    cur_state.R(i, 1) = x[kRotationCol1Offset + i];
    cur_state.R(i, 2) = x[kRotationCol2Offset + i];
    cur_state.omega(i) = x[kOmegaStateOffset + i];
  }
  for (int i = 0; i < 4; i++)
  {
    cur_state.motor_rpm(i) = x[kMotorStateOffset + i];
  }
  ...
  for (int i = 0; i < 3; i++)
  {
    dxdt[kPositionStateOffset + i] = x_dot(i);
    dxdt[kVelocityStateOffset + i] = v_dot(i);
    dxdt[kRotationCol0Offset + i] = R_dot(i, 0);
    dxdt[kRotationCol1Offset + i] = R_dot(i, 1);
    dxdt[kRotationCol2Offset + i] = R_dot(i, 2);
    dxdt[kOmegaStateOffset + i] = omega_dot(i);
  }
  for (int i = 0; i < 4; i++)
  {
    dxdt[kMotorStateOffset + i] = motor_rpm_dot(i);
  }
  for (int i = 0; i < kInternalStateSize; ++i)
  {
    if (std::isnan(dxdt[i]))
...
Quadrotor::updateInternalState(void)
{
  for (int i = 0; i < 3; i++)
  {
    internal_state_[kPositionStateOffset + i] = state_.x(i);
    internal_state_[kVelocityStateOffset + i] = state_.v(i);
    internal_state_[kRotationCol0Offset + i] = state_.R(i, 0);
    internal_state_[kRotationCol1Offset + i] = state_.R(i, 1);
    internal_state_[kRotationCol2Offset + i] = state_.R(i, 2);
    internal_state_[kOmegaStateOffset + i] = state_.omega(i);
  }
  internal_state_[kMotorStateOffset + 0] = state_.motor_rpm(0);
  internal_state_[kMotorStateOffset + 1] = state_.motor_rpm(1);
  internal_state_[kMotorStateOffset + 2] = state_.motor_rpm(2);
  internal_state_[kMotorStateOffset + 3] = state_.motor_rpm(3);
}
