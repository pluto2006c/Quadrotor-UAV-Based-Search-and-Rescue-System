...
constexpr int kOdomQueueSize = 100;
constexpr int kImuQueueSize = 10;
constexpr double kDefaultSimulationRate = 1000.0;
constexpr double kDefaultOdomRate = 100.0;
} // namespace
...
main(int argc, char** argv)
{
  ...
  ros::Publisher  odom_pub = n.advertise<nav_msgs::Odometry>("odom", kOdomQueueSize);
  ros::Publisher  imu_pub  = n.advertise<sensor_msgs::Imu>("imu", kImuQueueSize);
  ros::Subscriber cmd_sub =
    n.subscribe("cmd", kOdomQueueSize, &cmd_callback, ros::TransportHints().tcpNoDelay());
  ros::Subscriber f_sub =
    n.subscribe("force_disturbance", kOdomQueueSize, &force_disturbance_callback,
                ros::TransportHints().tcpNoDelay());
  ros::Subscriber m_sub =
    n.subscribe("moment_disturbance", kOdomQueueSize, &moment_disturbance_callback,
                ros::TransportHints().tcpNoDelay());
  ...
  n.param("rate/simulation", simulation_rate, kDefaultSimulationRate);
  ROS_ASSERT(simulation_rate > 0);

  double odom_rate;
  n.param("rate/odom", odom_rate, kDefaultOdomRate);
