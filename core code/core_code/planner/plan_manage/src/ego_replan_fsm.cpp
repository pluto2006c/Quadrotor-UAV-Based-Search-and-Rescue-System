...
namespace ego_planner
{
  namespace
  {
    constexpr double kExecTimerPeriod = 0.01;
    constexpr double kSafetyTimerPeriod = 0.05;
    constexpr double kSpinWaitSleep = 0.001;
    constexpr int kStartupSpinCycles = 1000;
    constexpr double kGlobalDisplayStep = 0.1;
    constexpr double kRejectedWaypointZ = -0.1;
    constexpr double kRemoteStartTimeTolerance = 0.25;

    string swarmTrajTopicName(int drone_id)
    {
      return string("/drone_") + std::to_string(drone_id) + string("_planning/swarm_trajs");
    }
  } // namespace
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    /* callback */
    exec_timer_ = nh.createTimer(ros::Duration(kExecTimerPeriod), &EGOReplanFSM::execFSMCallback, this);
    safety_timer_ = nh.createTimer(ros::Duration(kSafetyTimerPeriod), &EGOReplanFSM::checkCollisionCallback, this);

    odom_sub_ = nh.subscribe("odom_world", 1, &EGOReplanFSM::odometryCallback, this);

    if (planner_manager_->pp_.drone_id >= 1)
    {
      string sub_topic_name = swarmTrajTopicName(planner_manager_->pp_.drone_id - 1);
      swarm_trajs_sub_ = nh.subscribe(sub_topic_name.c_str(), 10, &EGOReplanFSM::swarmTrajsCallback, this, ros::TransportHints().tcpNoDelay());
    }
    string pub_topic_name = swarmTrajTopicName(planner_manager_->pp_.drone_id);
    ...
    else if (target_type_ == TARGET_TYPE::PRESET_TARGET)
    {
      ...
      int count = 0;
      while (ros::ok() && count++ < kStartupSpinCycles)
      {
        ros::spinOnce();
        ros::Duration(kSpinWaitSleep).sleep();
      }
  ...
  void EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp)
  {
    ...
    if (success)
    {
      ...
      /*** display ***/
      int i_end = floor(planner_manager_->global_data_.global_duration_ / kGlobalDisplayStep);
      vector<Eigen::Vector3d> gloabl_traj(i_end);
      for (int i = 0; i < i_end; i++)
      {
        gloabl_traj[i] = planner_manager_->global_data_.global_traj_.evaluate(i * kGlobalDisplayStep);
      }
      ...
      // visualization_->displayGoalPoint(end_pt_, Eigen::Vector4d(1, 0, 0, 1), 0.3, 0);
      visualization_->displayGlobalPathList(gloabl_traj, kGlobalDisplayStep, 0);
    }
    else
    {
      ROS_ERROR("Unable to generate global trajectory!");
  ...
  {
    if (msg->pose.position.z < kRejectedWaypointZ)
      return;
  ...
  void EGOReplanFSM::BroadcastBsplineCallback(const traj_utils::BsplinePtr &msg)
  {
    ...
    if (abs((ros::Time::now() - msg->start_time).toSec()) > kRemoteStartTimeTolerance)
    {
