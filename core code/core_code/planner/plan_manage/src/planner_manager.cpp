...
namespace ego_planner
{
  namespace
  {
    constexpr double kCloseGoalDistance = 0.2;
    constexpr double kNearTargetDistance = 0.1;
    constexpr double kNominalSamplingScale = 1.5;
    constexpr double kNearTargetSamplingScale = 5.0;
    constexpr double kLargeInitialCost = 999999.0;
    constexpr double kPathDisplayScale = 0.2;

    Eigen::Vector3i defaultAStarPoolSize()
    {
      return Eigen::Vector3i(100, 100, 100);
    }
  } // namespace
  ...
  void EGOPlannerManager::initPlanModules(ros::NodeHandle &nh, PlanningVisualization::Ptr vis)
  {
    ...
    bspline_optimizer_->a_star_->initGridMap(grid_map_, defaultAStarPoolSize());
                                        ...
                                        Eigen::Vector3d local_target_vel, bool flag_polyInit, bool flag_randomPolyTraj)
  {
    ...
    if ((start_pt - local_target_pt).norm() < kCloseGoalDistance)
    {
    ...
    /*** STEP 1: INIT ***/
    double ts = (start_pt - local_target_pt).norm() > kNearTargetDistance
                    ? pp_.ctrl_pt_dist / pp_.max_vel_ * kNominalSamplingScale
                    : pp_.ctrl_pt_dist / pp_.max_vel_ * kNearTargetSamplingScale; // pp_.ctrl_pt_dist / pp_.max_vel_ is too tense, and will surely exceed the acc/vel limits
    ...
    do
    {
      ...
      if (flag_first_call || flag_polyInit || flag_force_polynomial /*|| ( start_pt - local_target_pt ).norm() < 1.0*/) // Initial path generated from a min-snap traj by order.
      {
        ...
        double t;
        bool flag_too_far;
        ts *= kNominalSamplingScale; // ts will be divided by kNominalSamplingScale in the next
        do
        {
          ts /= kNominalSamplingScale;
          point_set.clear();
          flag_too_far = false;
          ...
          for (t = 0; t < time; t += ts)
          {
            ...
            if ((last_pt - pt).norm() > pp_.ctrl_pt_dist * kNominalSamplingScale)
            {
              flag_too_far = true;
              break;
            }
      ...
      else // Initial path generated from previous trajectory.
      {
        ...
        double cps_dist = pp_.ctrl_pt_dist * kNominalSamplingScale; // cps_dist will be divided by kNominalSamplingScale in the next
        size_t id = 0;
        do
        {
          cps_dist /= kNominalSamplingScale;
          point_set.clear();
          sample_length = 0;
          id = 0;
    ...
    if (pp_.use_distinctive_trajs)
    {
      ...
      double final_cost, min_cost = kLargeInitialCost;
      ...
      visualization_->displayMultiInitPathList(vis_trajs, kPathDisplayScale); // This visuallization will take up several milliseconds.
    }
    else
    {
      ...
      t_opt = ros::Time::now() - t_start;
      //static int vis_id = 0;
      visualization_->displayInitPathList(point_set, kPathDisplayScale, 0);
    }
