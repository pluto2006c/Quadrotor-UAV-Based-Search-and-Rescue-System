...
namespace ego_planner
{
  namespace
  {
    constexpr double kMinAlpha = 1e-5;

    void applyColor(visualization_msgs::Marker &marker, const Eigen::Vector4d &color)
    {
      marker.color.r = color(0);
      marker.color.g = color(1);
      marker.color.b = color(2);
      marker.color.a = color(3) > kMinAlpha ? color(3) : 1.0;
    }

    geometry_msgs::Point toPoint(const Eigen::Vector3d &pt)
    {
                                                ...
                                                Eigen::Vector4d color, int id, bool show_sphere /* = true */ )
  {
    ...
    {
      geometry_msgs::Point pt = toPoint(list[i]);
                                                        ...
                                                        const vector<Eigen::Vector3d> &list, double scale, Eigen::Vector4d color, int id)
  {
    ...
    for (int i = 0; i < int(list.size() / 2); i++)
    {
      ...
      start = toPoint(list[2 * i]);
      end = toPoint(list[2 * i + 1]);
      arrow.points.clear();
