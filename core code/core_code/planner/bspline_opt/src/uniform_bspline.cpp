...
namespace ego_planner
{
  namespace
  {
    constexpr int kTimeStretchAnchor = 5;

    double clampToSpan(double value, double lower, double upper)
    {
  ...
  Eigen::VectorXd UniformBspline::evaluateDeBoor(const double &u)
  {
    ...
    double ub = clampToSpan(u, u_(p_), u_(m_ - p_));
  ...
  {
    int num1 = kTimeStretchAnchor;
    int num2 = getKnot().rows() - 1 - kTimeStretchAnchor;
