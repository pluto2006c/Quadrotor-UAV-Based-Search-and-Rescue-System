...
namespace ego_planner
{
  namespace
  {
    constexpr int kMaxDistinctiveTrajs = 8;
    constexpr int kDistinctiveVariants = 2;
  } // namespace
  ...
  std::vector<ControlPoints> BsplineOptimizer::distinctiveTrajs(vector<std::pair<int, int>> segments)
  {
    ...
    int seg_upbound =
        std::min((int)segments.size(), static_cast<int>(floor(log(kMaxDistinctiveTrajs) / log(kDistinctiveVariants))));
    std::vector<ControlPoints> control_pts_buf;
    control_pts_buf.reserve(kMaxDistinctiveTrajs);
    ...
    RichInfoSegs.reserve(seg_upbound);
