...
namespace {
bool isInsideBounds(int x, int y, int z, const Eigen::Vector3d& min, const Eigen::Vector3d& max) {
  return x >= min.x() && x < max.x() && y >= min.y() && y < max.y() && z >= min.z() && z < max.z();
}

void advanceRayCell(int& x, int& y, int& z, int stepX, int stepY, int stepZ, double& tMaxX,
                    double& tMaxY, double& tMaxZ, double tDeltaX, double tDeltaY,
                    double tDeltaZ) {
  if (tMaxX < tMaxY) {
    if (tMaxX < tMaxZ) {
      x += stepX;
...
void Raycast(const Eigen::Vector3d& start, const Eigen::Vector3d& end, const Eigen::Vector3d& min,
  ...
  double dist = 0;
  while (true) {
    if (isInsideBounds(x, y, z, min, max)) {
    ...
    advanceRayCell(x, y, z, stepX, stepY, stepZ, tMaxX, tMaxY, tMaxZ, tDeltaX, tDeltaY,
                   tDeltaZ);
  }
}
...
bool RayCaster::step(Eigen::Vector3d& ray_pt) {
  ...
  // chooses the closest cube boundary. Only the first case of the four
  // has been commented in detail.
  advanceRayCell(x_, y_, z_, stepX_, stepY_, stepZ_, tMaxX_, tMaxY_, tMaxZ_, tDeltaX_,
                 tDeltaY_, tDeltaZ_);
