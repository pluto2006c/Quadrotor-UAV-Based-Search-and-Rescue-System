...
namespace {
std::string poseTopicName(int obj_id) {
  return "/dynamic/pose_" + std::to_string(obj_id);
}
}  // namespace
...
void ObjPredictor::init() {
  ...
  scale_init_.assign(obj_num_, false);
  obj_histories_.reserve(obj_num_);
  pose_subs_.reserve(obj_num_);
  ...
  for (int i = 0; i < obj_num_; i++) {
    ...
    ros::Subscriber pose_sub = node_handle_.subscribe<geometry_msgs::PoseStamped>(
        poseTopicName(i), 10, &ObjHistory::poseCallback, obj_his.get());
